...
`echo` is a deterministic placeholder (no model) so the pipeline runs with zero setup. Everything
below is a real model. Whatever it returns, the validator still snaps colors to the palette, drops
disallowed elements, removes text, and caps complexity — so even a small model stays on-aesthetic.

> Tip: prefer instruction-following / coding models for SVG. Bigger models draw cleaner; smaller
> ones are messier but the validator cleans up the palette and primitives.
...
One API key gives access to hundreds of models (Grok, Gemini, Llama, …), which makes it the
easiest way to try the costlier/stronger tiers.
...
**Cheapest options:** for minimal text → simple line art you don't need a frontier model. Good
low-cost picks on OpenRouter are `google/gemini-2.0-flash-001` and the small Llamas
(e.g. `meta-llama/llama-3.3-70b-instruct`, or the tiny `meta-llama/llama-3.2-3b-instruct` for
near-free). Model ids and prices change — check <https://openrouter.ai/models> for the current
list (and any free-tier models).
...
- A future **model ladder** will chain providers (local first, escalate to remote); the building
  block (`ModelLadder`) already exists in `core`.
